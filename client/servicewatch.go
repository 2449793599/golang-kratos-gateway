...
type watcherStatus struct {
	watcher           registry.Watcher            // 服务发现
	initializedChan   chan struct{}               // 初始化完成的通道
	selectedInstances []*registry.ServiceInstance // 当前发现的服务实例
}

type serviceWatcher struct {
	lock          sync.RWMutex
	watcherStatus map[string]*watcherStatus     // endpoint：针对某个ENDPOINT的服务发现
	appliers      map[string]map[string]Applier // nodeApplier
}
	...
	existed := func() bool {
		...
		func() {
			...
			go func() {
				...
				services, err := watcher.Next() // 返回所有可用的服务实例
...
func (s *serviceWatcher) DebugHandler() http.Handler {
	...
	debugMux.HandleFunc("/debug/watcher/nodes", func(w http.ResponseWriter, r *http.Request) { // 获取指定服务的多有可用节点
